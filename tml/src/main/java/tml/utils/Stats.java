...
public String toString() {
    ...
    calculateDerived();
    return
      "Count   " + doubleToString(count, 8) + '\n'
      + "Min     " + doubleToString(min, 8) + '\n'
      + "Max     " + doubleToString(max, 8) + '\n'
      + "Sum     " + doubleToString(sum, 8) + '\n'
      + "SumSq   " + doubleToString(sumSq, 8) + '\n'
      + "Mean    " + doubleToString(mean, 8) + '\n'
      + "StdDev  " + doubleToString(stdDev, 8) + '\n';
  }
  
  private String doubleToString(double d, int size) {
	  StringBuffer buff = new StringBuffer();
	  DecimalFormat df = new DecimalFormat("0.000");
	  buff.append(df.format(d));
