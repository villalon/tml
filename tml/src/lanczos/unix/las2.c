...

{
   ...
   /* open files for input/output */
    ...
    /* to get a _random starting vector, the first n cells must be
     * initialize to zero */
    for (i = 0; i < n; i++) r[i] = 0.;
    ...
    if (vectors) {
	  ...
	  /* write left s-vector to output file */
        printf("Printing vector xv1 from %d size %d\n", ida, size1);
...

{
   ...
   printf("Printing n %d size %d\n", n, sizeof(n));
   write(fp_out2, (char *)&n, sizeof(n));
   printf("Printing js %d size %d\n", js, sizeof(js));
   write(fp_out2, (char *)&js, sizeof(js));
   printf("Printing kappa %d size %d\n", kappa, sizeof(kappa));
//   write(fp_out2, (char *)&kappa, 8);
...

{
   ...
   /* get initial vector; default is _random */
   rnm = startv(n, wrkptr);
...
/***********************************************************************
   ...
   This is a translation of a Fortran-77 uniform _random number
   ...
   _random.  The calling program should  not  alter  the value of the
   argument between subsequent calls to _random.  Random returns values
   ...
   (output)
   _random  a double precision _random number between (0,1)
